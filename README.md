...
## <div align="center">SAR Ship Detection Optimization</div>

This repository includes three key innovative modules specifically designed for SAR ship detection:

### 1. Frequency-Spatial Enhancement Module (FSEM)
- **Purpose**: Adaptive noise suppression and structure preservation
- **Features**: Dual-branch architecture with edge enhancement and frequency-domain filtering
- **Benefits**: Improves ship boundary clarity and reduces sea clutter interference

### 2. Multi-Stage Feature Enhancement (MSFE)
- **Purpose**: Scattering-aware feature stabilization
- **Features**: Polarized attention, dynamic normalization, and edge-enhanced FFN
- **Benefits**: Stabilizes training under strong backscattering and amplifies ship signatures

### 3. Small-Object Enhance Pyramid (SOEP)
- **Purpose**: Efficient small-target recovery
- **Features**: Space-to-Depth convolution and multi-scale feature fusion
- **Benefits**: Improves small ship detection without extra computational overhead

### Usage Example

```bash
# Train with optimized SAR ship detection modules
yolo train model=FSDD/cfg/models/COMP.yaml data=sar_ship.yaml epochs=100 batch=16

# Predict with optimized model
yolo predict model=best.pt source=sar_images/
```

### Configuration

The optimized model configuration is available at `FSDD/cfg/models/COMP.yaml`, which includes all three enhancement modules.
